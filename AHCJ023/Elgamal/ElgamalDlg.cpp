                   ...
                   /* length of input block */
{
  ...
  index = (unsigned int)((context->count[0] >> 3) & 0x3F);//字节数

  /* Update number of bits */
  if ((context->count[0] += ((UINT4)inputLen << 3))//inputLen << 3变成字位数
   < ((UINT4)inputLen << 3))
 context->count[1]++;
  context->count[1] += ((UINT4)inputLen >> 29);
//inputlen 放到count中 count中为字节数
...
void CElgamalDlg::OnButton1() 
{
    ...
    if (file == NULL)
	{
	    MessageBox("打不开文件或无此文件,请检查输入的文件");
	    return ;
	}
	else 
	{
	...
	void CElgamalDlg::OnButton2() 
{
		...
		m_x4.Format("%d",m_x1);
		UpdateData(false);
		MessageBox("密钥生成成功，请保管好!");
}


int CElgamalDlg::gcd(int a ,int b)//最大公约数
{
	int r;
	r=a%b;
	if(r==0)
...
int CElgamalDlg::momi(int a,int e,int n)//求模幂，从左向右
{	
...
int  CElgamalDlg::moni(int d,int n)	//求模逆
{
 ...
 bool CElgamalDlg::prime(int n,int z)//判断素数
 {
 		int a,r,k1,k2;
 		bool falt=1;
 		k1=n-1;
...
void CElgamalDlg::OnButton5() 
{
	...
	UpdateData(false);
	if(m_v1==m_v2)
        MessageBox("v1=v2 ,验证成功!");
	else
		MessageBox("v1！=v2,验证失败");
